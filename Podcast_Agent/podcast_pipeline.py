...
def generate_audio(
    ...
    logger.info(f"Generated dialogue script successfully.")
    ...
    temporary_file = NamedTemporaryFile(dir=CACHE_DIR, delete=False, suffix=".mp3")
    combined_audio.export(temporary_file.name, format="mp3")

    for file in glob.glob(f"{CACHE_DIR}*.mp3"):
        if os.path.isfile(file) and time.time() - os.path.getmtime(file) > CLEAR_CACHE_OLDER_THAN:
            os.remove(file)
